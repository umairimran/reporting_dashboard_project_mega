...
class VibeUploadHandler:
    """Handler for Vibe CSV file uploads and processing."""
    ...
    def _get_upload_directory(self, client_id: uuid.UUID) -> str:
        ...
        upload_dir = os.path.join(settings.UPLOAD_DIR, 'vibe', str(client_id))
    ...
    async def process_upload(
        ...
        """
        Process uploaded Vibe CSV file.
        ...
        Returns:
            UploadedFile record
        """
        logger.info(f"Processing Vibe upload for client {client_name}: {file.filename}")
        ...
        try:
            ...
            # Parse file
            logger.info(f"Parsing Vibe file: {file.filename}")
            raw_records = VibeParser.parse_file(file_path)
            uploaded_file.records_count = len(raw_records)
            self.db.commit()
            
            logger.info(f"Parsed {len(raw_records)} records from Vibe file")
            
            # Determine run_date from data (use most common date or today)
            run_date = date.today()
            if raw_records:
                # Try to extract date from first record
                try:
                    from app.etl.transformer import TransformerService
                    first_date = TransformerService.parse_date(raw_records[0].get('impression_date'))
                    run_date = first_date
                except:
                    pass
            ...
            logger.info(f"Vibe upload processing completed: {uploaded_file.upload_status}")
            
        except Exception as e:
            uploaded_file.upload_status = 'failed'
            uploaded_file.error_message = str(e)
            uploaded_file.processed_at = datetime.utcnow()
            logger.error(f"Vibe upload processing failed: {str(e)}", exc_info=True)
